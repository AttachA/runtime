	...
	AttachAFun(funs_ConditionVariable_wait, 1,{
		...
		case 2:{
			if (args[1].meta.vtype == VType::struct_) {
				...
				art::unique_lock lock(unif, art::adopt_lock);
				class_.wait(lock);
				lock.release();
				break;
			}
...
#pragma endregion
	...
	ValueItem* createThread(ValueItem* vals, uint32_t len) {
		...
		if (len != 1) {
			...
			while (len--)
				copyArgs[i++] = *vals++;
			art::thread([](typed_lgr<FuncEnvironment> func, ValueItem* args, uint32_t len) {
			...
			art::thread([](typed_lgr<FuncEnvironment> func) {
	...
	ValueItem* createThreadAndWait(ValueItem* vals, uint32_t len) {
		...
		TaskMutex mtx;
		MutexUnify unif(mtx);
		art::unique_lock ul(unif);
		bool end = false;
		ValueItem* res = nullptr;
		typed_lgr<FuncEnvironment> func = *vals->funPtr();
		if (len != 1) {
			art::thread([&end, &res, &mtx, &cv](typed_lgr<FuncEnvironment> func, ValueItem* args, uint32_t len) {
				try{
					auto tmp = FuncEnvironment::sync_call(func, args, len);
					art::unique_lock ul(mtx);
					res = tmp;
					end = true;
					cv.notify_all();
				}catch(...){
			...
			art::thread([&end, &res, &mtx, &cv](typed_lgr<FuncEnvironment> func) {
				try{
					auto tmp = FuncEnvironment::sync_call(func, nullptr, 0);
					art::unique_lock ul(mtx);
					res = tmp;
	...
	ValueItem* _createAsyncThread__Awaiter(ValueItem* val, uint32_t len){
		...
		art::unique_lock<MutexUnify> ul(awaiter->unif);
		try{
			while(!awaiter->end)
				awaiter->cv.wait(ul);
		}catch(...){
	...
	ValueItem* createAsyncThread(ValueItem* vals, uint32_t len){
		...
		try{
			...
			if (len != 1) {
				...
				art::thread([awaiter](typed_lgr<FuncEnvironment> func, ValueItem* args, uint32_t len) {
					try{
						auto tmp = FuncEnvironment::sync_call(func, args, len);
						art::unique_lock ul(awaiter->mtx);
						awaiter->res = tmp;
						awaiter->end = true;
				...
				art::thread([awaiter](typed_lgr<FuncEnvironment> func) {
					try{
						auto tmp = FuncEnvironment::sync_call(func, nullptr, 0);
						art::unique_lock ul(awaiter->mtx);
						awaiter->res = tmp;
	...
	namespace atomic{
		...
		struct AtomicObject{
			...
			AtomicObject& operator=(const AtomicObject& other){
				art::unique_lock<TaskRecursiveMutex> olock(const_cast<AtomicObject&>(other).mutex);
			...
			AtomicObject& operator=(AtomicObject&& other){
				art::unique_lock<TaskRecursiveMutex> olock(other.mutex);
