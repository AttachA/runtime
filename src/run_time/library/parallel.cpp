...
namespace art{
	...
	#pragma region ConditionVariable
		...
		void init_ConditionVariable() {
			define_ConditionVariable = CXX::Interface::createTable<typed_lgr<TaskConditionVariable>>("condition_variable",
	...
	#pragma region Mutex
		...
		void init_Mutex() {
			define_Mutex = CXX::Interface::createTable<typed_lgr<TaskMutex>>("mutex",
	...
	#pragma region RecursiveMutex
		...
		void init_RecursiveMutex() {
			define_RecursiveMutex = CXX::Interface::createTable<typed_lgr<TaskRecursiveMutex>>("recursive_mutex",
				CXX::Interface::direct_method("lock", funs_Mutex_lock),
				CXX::Interface::direct_method("unlock", funs_Mutex_unlock),
				CXX::Interface::direct_method("try_lock", funs_Mutex_try_lock),
				CXX::Interface::direct_method("try_lock_until", funs_Mutex_try_lock_until),
	...
	#pragma region Semaphore
		...
		void init_Semaphore() {
			define_Semaphore = CXX::Interface::createTable<typed_lgr<TaskSemaphore>>("semaphore",
	...
	#pragma region TaskQuery
		...
		void init_TaskQuery() {
			define_TaskQuery = CXX::Interface::createTable<typed_lgr<TaskQuery>>("task_query",
	...
	#pragma region TaskResultIterator
		...
		void init_TaskResultIterator() {
			define_TaskResultIterator = CXX::Interface::createTable<TaskResultIterator>("task_result_iterator",
	...
	#pragma region Task
		AttachAFun(funs_Task_start, 1, {
			Task::start(CXX::Interface::getExtractAs<typed_lgr<Task>>(args[0], define_Task));
		})
		AttachAFun(funs_Task_yield_iterate, 1, {
			return Task::yield_iterate(CXX::Interface::getExtractAs<typed_lgr<Task>>(args[0], define_Task));
		})
		AttachAManagedFun(funs_Task_get_result, 1, {
			if(len >= 2)
				return Task::get_result(CXX::Interface::getExtractAs<typed_lgr<Task>>(args[0], define_Task), (size_t)args[1]);
			else
				return Task::get_result(CXX::Interface::getExtractAs<typed_lgr<Task>>(args[0], define_Task));
		})
		AttachAFun(funs_Task_has_result, 1, {
			if(len >= 2)
				return Task::has_result(CXX::Interface::getExtractAs<typed_lgr<Task>>(args[0], define_Task), (size_t)args[1]);
			else
				return Task::has_result(CXX::Interface::getExtractAs<typed_lgr<Task>>(args[0], define_Task));
		})
		AttachAFun(funs_Task_await_task, 1, {
			if(len >= 2)
				Task::await_task(CXX::Interface::getExtractAs<typed_lgr<Task>>(args[0], define_Task), (bool)args[1]);
			else
				Task::await_task(CXX::Interface::getExtractAs<typed_lgr<Task>>(args[0], define_Task));
		})
		AttachAFun(funs_Task_await_results, 1, {
			return Task::await_results(CXX::Interface::getExtractAs<typed_lgr<Task>>(args[0], define_Task));
		})
		AttachAFun(funs_Task_notify_cancel, 1, {
			Task::notify_cancel(CXX::Interface::getExtractAs<typed_lgr<Task>>(args[0], define_Task));
		})
		AttachAFun(funs_Task_size, 1, {
			Task& task = *CXX::Interface::getExtractAs<typed_lgr<Task>>(args[0], define_Task);
		...
		void init_Task(){
			define_Task = CXX::Interface::createTable<typed_lgr<Task>>("task",
				...
				CXX::Interface::direct_method(symbols::structures::convert::to_function, funs_Task_to_<typed_lgr<FuncEnvironment>&>),
			...
			CXX::Interface::typeVTable<typed_lgr<Task>>() = define_Task;
		}
		
	#pragma endregion
	#pragma region TaskGroup
		AttachAFun(funs_TaskGroup_start, 1, {
			Task::start(CXX::Interface::getExtractAs<list_array<typed_lgr<Task>>>(args[0], define_TaskGroup));
		})
		AttachAFun(funs_TaskGroup_await_multiple, 1, {
			switch (len) {
			case 1:
				Task::await_multiple(CXX::Interface::getExtractAs<list_array<typed_lgr<Task>>>(args[0], define_TaskGroup));
				break;
			case 2:
				Task::await_multiple(CXX::Interface::getExtractAs<list_array<typed_lgr<Task>>>(args[0], define_TaskGroup), (bool)args[1]);
				break;
			case 3:
			default:{
				bool release = (bool)args[2];
				Task::await_multiple(CXX::Interface::getExtractAs<list_array<typed_lgr<Task>>>(args[0], define_TaskGroup), (bool)args[1], release);
				if(release)
					CXX::Interface::getExtractAs<list_array<typed_lgr<Task>>>(args[0], define_TaskGroup).clear();
				break;
			}
			}
		})
		AttachAFun(funs_TaskGroup_await_results, 1, {
			return Task::await_results(CXX::Interface::getExtractAs<list_array<typed_lgr<Task>>>(args[0], define_TaskGroup));
		})
		AttachAFun(funs_TaskGroup_notify_cancel, 1, {
			Task::notify_cancel(CXX::Interface::getExtractAs<list_array<typed_lgr<Task>>>(args[0], define_TaskGroup));
		})

		template<typename T>
		AttachAFun(funs_TaskGroup_array_to_, 1, {
			auto results = Task::await_results(CXX::Interface::getExtractAs<list_array<typed_lgr<Task>>>(args[0], define_TaskGroup));
		...
		AttachAFun(funs_TaskGroup_to_set, 1, {
			std::unordered_set<ValueItem> res;
			for(auto& i : Task::await_results(CXX::Interface::getExtractAs<list_array<typed_lgr<Task>>>(args[0], define_TaskGroup)))
				res.insert(i);
			return res;
		})
				...
				case VType::struct_:
					{
						...
						if(str.get_vtable() == define_Task){
							tasks.push_back(CXX::Interface::getAs<typed_lgr<Task>>(str));
							break;
						}else if (str.get_vtable() == define_TaskGroup){
							tasks.push_back(CXX::Interface::getAs<list_array<typed_lgr<Task>>>(str));
							break;
						}else
							break;
					}
		...
		AttachAFun(funs_TaskGroup_add, 1, {
			auto& tasks = CXX::Interface::getExtractAs<list_array<typed_lgr<Task>>>(args[0], define_TaskGroup);
			for(uint32_t i = 0; i < len; i++)
				___createProxy_TaskGroup__push_item(tasks, args[i]);
		})
		void init_TaskGroup() {
			define_TaskGroup = CXX::Interface::createTable<list_array<typed_lgr<Task>>>("task_group",
			...
			CXX::Interface::typeVTable<list_array<typed_lgr<Task>>>() = define_TaskGroup;
		}
	#pragma endregion
		...
		namespace constructor {
			...
			AttachAFun(construct_Task, 1, {
				if(args[0].meta.vtype == VType::async_res)
					return ValueItem(CXX::Interface::constructStructure<typed_lgr<Task>>(define_Task, (typed_lgr<Task>&)args[0]), no_copy);
					...
					return ValueItem(CXX::Interface::constructStructure<typed_lgr<Task>>(define_Task, new Task(func,values, used_task_local,fault_func, timeout)), no_copy);
				}
				else
					return ValueItem(CXX::Interface::constructStructure<typed_lgr<Task>>(define_Task, CXX::Interface::getExtractAs<typed_lgr<Task>>(args[0],define_Task)), no_copy);
			})
			ValueItem* createProxy_TaskGroup(ValueItem* val, uint32_t len) {
				if(!len)
					return new ValueItem(CXX::Interface::constructStructure<list_array<typed_lgr<Task>>>(define_TaskGroup), no_copy);
				else{
					list_array<typed_lgr<Task>> tasks;
					tasks.reserve_push_back(len);
					for(uint32_t i = 0; i < len; i++)
						___createProxy_TaskGroup__push_item(tasks, val[i]);
					return new ValueItem(CXX::Interface::constructStructure<list_array<typed_lgr<Task>>>(define_TaskGroup, std::move(tasks)), no_copy);
				}
			}
		}
		...
		ValueItem* createThread(ValueItem* vals, uint32_t len) {
			...
			if (len != 1) {
				...
				while (len--)
					copyArgs[i++] = *vals++;
				thread([](typed_lgr<FuncEnvironment> func, ValueItem* args, uint32_t len) {
				...
				thread([](typed_lgr<FuncEnvironment> func) {
		...
		ValueItem* createThreadAndWait(ValueItem* vals, uint32_t len) {
			...
			typed_lgr<FuncEnvironment> func = *vals->funPtr();
			if (len != 1) {
				thread([&end, &res, &mtx, &cv](typed_lgr<FuncEnvironment> func, ValueItem* args, uint32_t len) {
					try{
						auto tmp = FuncEnvironment::sync_call(func, args, len);
						unique_lock ul(mtx);
						res = tmp;
				...
				thread([&end, &res, &mtx, &cv](typed_lgr<FuncEnvironment> func) {
					try{
						auto tmp = FuncEnvironment::sync_call(func, nullptr, 0);
						unique_lock ul(mtx);
						res = tmp;
		...
		ValueItem* createAsyncThread(ValueItem* vals, uint32_t len){
			...
			try{
				...
				if (len != 1) {
					...
					thread([awaiter](typed_lgr<FuncEnvironment> func, ValueItem* args, uint32_t len) {
						try{
							auto tmp = FuncEnvironment::sync_call(func, args, len);
							unique_lock ul(awaiter->mtx);
							awaiter->res = tmp;
							awaiter->end = true;
					...
					thread([awaiter](typed_lgr<FuncEnvironment> func) {
						try{
							auto tmp = FuncEnvironment::sync_call(func, nullptr, 0);
							unique_lock ul(awaiter->mtx);
							awaiter->res = tmp;
		...
		namespace atomic{
			...
			public:
				...
				static void init(){
					if constexpr (std::is_integral_v<T> && !std::is_floating_point_v<T> && !std::is_same_v<T,bool>){
						...
						virtual_table = CXX::Interface::createTable<AtomicBasic<T>>(type_name,
							...
							CXX::Interface::direct_method(symbols::structures::bitwise_and_operator, __and),
							CXX::Interface::direct_method(symbols::structures::bitwise_or_operator, __or),
							CXX::Interface::direct_method(symbols::structures::bitwise_xor_operator, __xor),
							CXX::Interface::direct_method(symbols::structures::increment_operator, __inc),
							CXX::Interface::direct_method(symbols::structures::decrement_operator, __dec),
							CXX::Interface::direct_method(symbols::structures::not_equal_operator, __not_equal),
							CXX::Interface::direct_method(symbols::structures::equal_operator, __equal),
							CXX::Interface::direct_method(symbols::structures::less_operator, __less),
							CXX::Interface::direct_method(symbols::structures::greater_operator, __greater),
							CXX::Interface::direct_method(symbols::structures::less_or_equal_operator, __less_or_equal),
							CXX::Interface::direct_method(symbols::structures::greater_or_equal_operator, __greater_or_equal),
							CXX::Interface::direct_method(symbols::structures::not_operator, __not),
							CXX::Interface::direct_method(symbols::structures::bitwise_not_operator, __bitwise_not),
							CXX::Interface::direct_method(symbols::structures::convert::to_string, __to_string),
							CXX::Interface::direct_method(symbols::structures::convert::to_ui8, __to_ui8),
							CXX::Interface::direct_method(symbols::structures::convert::to_ui16, __to_ui16),
							CXX::Interface::direct_method(symbols::structures::convert::to_ui32, __to_ui32),
							CXX::Interface::direct_method(symbols::structures::convert::to_ui64, __to_ui64),
							CXX::Interface::direct_method(symbols::structures::convert::to_i8, __to_i8),
							CXX::Interface::direct_method(symbols::structures::convert::to_i16, __to_i16),
							CXX::Interface::direct_method(symbols::structures::convert::to_i32, __to_i32),
							CXX::Interface::direct_method(symbols::structures::convert::to_i64, __to_i64),
							CXX::Interface::direct_method(symbols::structures::convert::to_float, __to_float),
							CXX::Interface::direct_method(symbols::structures::convert::to_double, __to_double),
							CXX::Interface::direct_method(symbols::structures::convert::to_boolean, __to_boolean),
						...
						virtual_table = CXX::Interface::createTable<AtomicBasic<T>>(std::is_same_v<T, float> ? "atomic_float" : "atomic_double",
							CXX::Interface::direct_method(symbols::structures::add_operator, __add),
							CXX::Interface::direct_method(symbols::structures::subtract_operator, __sub),
							CXX::Interface::direct_method(symbols::structures::increment_operator, __inc),
							CXX::Interface::direct_method(symbols::structures::decrement_operator, __dec),
						...
						virtual_table = CXX::Interface::createTable<AtomicBasic<T>>("atomic_boolean",
							CXX::Interface::direct_method(symbols::structures::not_equal_operator, __not_equal),
							CXX::Interface::direct_method(symbols::structures::equal_operator, __equal),
							CXX::Interface::direct_method(symbols::structures::not_operator, __not),
							CXX::Interface::direct_method(symbols::structures::bitwise_not_operator, __bitwise_not),
			...
			struct AtomicObject{
				...
				static void init(){
					virtual_table = CXX::Interface::createTable<AtomicObject>("AtomicAny",
						CXX::Interface::direct_method(symbols::structures::add_operator, __add),
						CXX::Interface::direct_method(symbols::structures::subtract_operator, __sub),
						CXX::Interface::direct_method(symbols::structures::multiply_operator, __mul),
						CXX::Interface::direct_method(symbols::structures::divide_operator, __div),
						CXX::Interface::direct_method(symbols::structures::modulo_operator, __mod),
						CXX::Interface::direct_method(symbols::structures::bitwise_and_operator, __and),
						CXX::Interface::direct_method(symbols::structures::bitwise_or_operator, __or),
						CXX::Interface::direct_method(symbols::structures::bitwise_xor_operator, __xor),
						CXX::Interface::direct_method(symbols::structures::bitwise_shift_left_operator, __lshift),
						CXX::Interface::direct_method(symbols::structures::bitwise_shift_right_operator, __rshift),
						CXX::Interface::direct_method(symbols::structures::not_equal_operator, __not_equal),
						CXX::Interface::direct_method(symbols::structures::equal_operator, __equal),
						CXX::Interface::direct_method(symbols::structures::less_operator, __less),
						CXX::Interface::direct_method(symbols::structures::greater_operator, __greater),
						CXX::Interface::direct_method(symbols::structures::less_or_equal_operator, __less_equal),
						CXX::Interface::direct_method(symbols::structures::greater_or_equal_operator, __greater_equal),
						CXX::Interface::direct_method(symbols::structures::increment_operator, __inc),
						CXX::Interface::direct_method(symbols::structures::decrement_operator, __dec),
						CXX::Interface::direct_method(symbols::structures::not_operator, __not),
						CXX::Interface::direct_method(symbols::structures::bitwise_not_operator, __not),
						CXX::Interface::direct_method(symbols::structures::convert::to_boolean, __to_boolean),
						CXX::Interface::direct_method(symbols::structures::convert::to_string, __to_string),
						CXX::Interface::direct_method(symbols::structures::convert::to_ui8, __to_u8),
						CXX::Interface::direct_method(symbols::structures::convert::to_ui16, __to_u16),
						CXX::Interface::direct_method(symbols::structures::convert::to_ui32, __to_u32),
						CXX::Interface::direct_method(symbols::structures::convert::to_ui64, __to_u64),
						CXX::Interface::direct_method(symbols::structures::convert::to_i8, __to_i8),
						CXX::Interface::direct_method(symbols::structures::convert::to_i16, __to_i16),
						CXX::Interface::direct_method(symbols::structures::convert::to_i32, __to_i32),
						CXX::Interface::direct_method(symbols::structures::convert::to_i64, __to_i64),
						CXX::Interface::direct_method(symbols::structures::convert::to_float, __to_float),
						CXX::Interface::direct_method(symbols::structures::convert::to_double, __to_double),
						CXX::Interface::direct_method(symbols::structures::convert::to_ui8_arr, __to_ui8_arr),
						CXX::Interface::direct_method(symbols::structures::convert::to_ui16_arr, __to_ui16_arr),
						CXX::Interface::direct_method(symbols::structures::convert::to_ui32_arr, __to_ui32_arr),
						CXX::Interface::direct_method(symbols::structures::convert::to_ui64_arr, __to_ui64_arr),
						CXX::Interface::direct_method(symbols::structures::convert::to_i8_arr, __to_i8_arr),
						CXX::Interface::direct_method(symbols::structures::convert::to_i16_arr, __to_i16_arr),
						CXX::Interface::direct_method(symbols::structures::convert::to_i32_arr, __to_i32_arr),
						...
						CXX::Interface::direct_method(symbols::structures::convert::to_map, __to_map),
						CXX::Interface::direct_method(symbols::structures::convert::to_set, __to_set),
						CXX::Interface::direct_method(symbols::structures::convert::to_uarr, __to_uarr),
						CXX::Interface::direct_method("explicit_await", __explicit_await),
						CXX::Interface::direct_method("make_gc", __make_gc),
						CXX::Interface::direct_method("localize_gc", __localize_gc),
						CXX::Interface::direct_method("ungc", __ungc),
						CXX::Interface::direct_method("is_gc", __is_gc),
						CXX::Interface::direct_method("hash", __hash),
						CXX::Interface::direct_method("make_slice", __make_slice),
						CXX::Interface::direct_method("size", __size),
						CXX::Interface::direct_method("get", __get),
						CXX::Interface::direct_method("set", __set)
					);
				}
