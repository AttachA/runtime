...
namespace art{
	namespace chanel {
		...
		bool AutoEventChanel::notify(const ValueItem& val) {
			if(!notifier_event.is_deleted()){
				switch (ntype) {
					...
					case NotifyType::async: return Task::get_result(notifier_event->async_notify(const_cast<ValueItem&>(val)));
		...
		AttachAFun(funs_Chanel_auto_event, 3, {
			art::CXX::Interface::getExtractAs<typed_lgr<Chanel>>(args[0],define_Chanel)->auto_event(
		...
		AttachAFun(funs_Chanel_remove_handle, 2, {
			art::CXX::Interface::getExtractAs<typed_lgr<Chanel>>(args[0],define_Chanel)->remove_handle(
				art::CXX::Interface::getExtractAs<typed_lgr<ChanelHandler>>(args[1],define_ChanelHandler)
			);
		})
		AttachAFun(funs_Chanel_remove_auto_notify, 2, {
			art::CXX::Interface::getExtractAs<typed_lgr<Chanel>>(args[0],define_Chanel)->remove_auto_notify(
				art::CXX::Interface::getExtractAs<typed_lgr<AutoNotifyChanel>>(args[1],define_AutoNotifyChanel)
			);
		})
		AttachAFun(funs_Chanel_remove_auto_event, 2, {
			art::CXX::Interface::getExtractAs<typed_lgr<Chanel>>(args[0],define_Chanel)->remove_auto_event(
				art::CXX::Interface::getExtractAs<typed_lgr<AutoEventChanel>>(args[1],define_AutoEventChanel)
			);
		})
		AttachAFun(funs_Chanel_add_handle, 2, {
			art::CXX::Interface::getExtractAs<typed_lgr<Chanel>>(args[0],define_Chanel)->add_handle(
				art::CXX::Interface::getExtractAs<typed_lgr<ChanelHandler>>(args[1],define_ChanelHandler)
			);
		})
