...
namespace art {
    namespace selector {
        ...
        public:
            SelectorInfo(SelectorInfo<T>* prev)
                : prev(prev) {}
        ...
        public:
            LASelectorInfo(list_array<T>&& select)
                : select(select), SelectorInfo<T>(nullptr) {}

            LASelectorInfo(const list_array<T>& select)
                : select(select), SelectorInfo<T>(nullptr) {}
            ...
            Selector<T>* getSelector() override {
                ...
                public:
                    LocalSelector(list_array<T>& select)
                        : curr(select.begin()), end(select.end()), can_iter(select.size()) {}
        ...
        public:
            RefsSelectorInfo(const list_array<T*>& select, SelectorInfo<T>* prev)
                : select(select), SelectorInfo<T>(prev) {}

            RefsSelectorInfo(const RefsSelectorInfo&)
                : select(select), SelectorInfo<T>(prev) {}

            RefsSelectorInfo(list_array<T*>&& select)
                : select(select), SelectorInfo<T>(nullptr) {}
        ...
        public:
            WhereSelectorInfo(SelectorInfo<T>* prev, std::function<bool(const T& it)> checker)
                : checker(checker), SelectorInfo<T>(prev) {}
        ...
        class JoinSelectorInfo : public RefsSelectorInfo<T> {
            ...
            JoinSelectorInfo(const list_array<T*>& select)
                : joinable(nullptr), RefsSelectorInfo<T>(select, nullptr) {}

        public:
            JoinSelectorInfo(SelectorInfo<T>* prev, SelectorInfo<T>* joinable)
                : joinable(joinable), RefsSelectorInfo<T>(prev->selectedRefArr().join_copy(prev->selectedRefArr()), prev) {}

            template <class FN>
            JoinSelectorInfo(SelectorInfo<T>* prev, SelectorInfo<T>* joinable, FN join_selector)
                : joinable(joinable), RefsSelectorInfo<T>(prev->selectedRefArr().join_copy(prev->selectedRefArr(), join_selector), prev) {}
        ...
        SelectorInfo<T>* Odd(SelectorInfo<T>* selector) {
            ...
            public:
                OddSelectorInfo(SelectorInfo<T>* prev)
                    : SelectorInfo<T>(prev) {}
        ...
        SelectorInfo<T>* Even(SelectorInfo<T>* selector) {
            class EvenSelectorInfo : public SelectorInfo<T>{
                public :
                    EvenSelectorInfo(SelectorInfo<T> * prev) : SelectorInfo<T>(prev){} list_array<T*> selectedRefArr() override{
                        list_array<T*> res;
            Selector<T>* select = getSelector();
            while (select->next())
                res.push_back(&select->current());
