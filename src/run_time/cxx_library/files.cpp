...
namespace art{
    namespace files{
        ...
        public:
            ...
            const bool is_read;
            const bool required_full;
            File_(NativeWorkerManager* manager, int handle, char* buffer, uint32_t buffer_size, uint64_t offset) : NativeWorkerHandle(manager), handle(handle), is_read(false), buffer_size(buffer_size), offset(offset), required_full(true){
                this->buffer = new char[buffer_size];
                memcpy(this->buffer, buffer, buffer_size);
            }
            
            File_(NativeWorkerManager* manager, int handle, uint32_t buffer_size, uint64_t offset, bool required_full = true) : NativeWorkerHandle(manager), handle(handle), is_read(true), buffer_size(buffer_size), offset(offset), required_full(required_full){
                this->buffer = new char[buffer_size];
            }
            ~File_(){
                if(buffer)
            ...
            bool error_filter(int error){
                switch (error) {
                    ...
                    case ENOMEM: exception(io_errors::no_enough_memory); return true;
                    case ENOBUFS: exception(io_errors::invalid_user_buffer); return true;
                    case EDQUOT: exception(io_errors::no_enough_quota); return true;
                    case ECANCELED:
                    case EINTR: return false;
                    case ESPIPE: exception(io_errors::eof); return true;
                    default: exception(io_errors::unknown_error); return true;
                }
            }
        };
        ...
        public:
            FileManager(const char* path, size_t path_len, open_mode open, on_open_action action, share_mode share, _sync_flags flags, pointer_mode _pointer_mode) noexcept(false) : _pointer_mode(_pointer_mode) {
                read_pointer = 0;
                write_pointer = 0;
                int mode = O_NONBLOCK;
        ...
        class FolderChangesMonitorImpl : public NativeWorkerManager, public IFolderChangesMonitor{
            ...
            void initialize() {
                constexpr uint32_t _listen_filters 
                    = IN_MODIFY 
                    | IN_CREATE 
                    | IN_DELETE 
                    | IN_ATTRIB 
