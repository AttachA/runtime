...
namespace art {
    ...
    struct task_callback {
        ...
        task_callback(ValueItem& args, void (*on_await)(ValueItem&) = dummy, void (*on_cancel)(ValueItem&) = dummy, void (*on_timeout)(ValueItem&) = dummy, void (*on_start)(ValueItem&) = dummy, void (*on_destruct)(ValueItem&) = dummy)
            : args(args), on_start(on_start), on_await(on_await), on_cancel(on_cancel), on_timeout(on_timeout), on_destruct(on_destruct) {}
