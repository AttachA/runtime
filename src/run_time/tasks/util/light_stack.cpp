...
namespace art {
    ...
    art::ustring dump_stack(bool* pPtr) {
        ...
        do {
            ...
            res += " State: ";
            res += fmt_state(stMemBasicInfo.State);
    ...
    struct allocation_details {
        ...
        allocation_details(size_t page_ptr, size_t base_ptr, size_t len, DWORD prot, DWORD state)
            : page((bool*)page_ptr), base((bool*)page_ptr), length(len), state(state), prot(prot) {}

        allocation_details(const MEMORY_BASIC_INFORMATION& mbi)
            : allocation_details((size_t)mbi.BaseAddress, (size_t)mbi.AllocationBase, mbi.RegionSize, mbi.Protect, mbi.State) {}
    };
    ...
    struct stack_snapshot {
        ...
        stack_snapshot()
            : stack_snapshot(GetStackPointer()) {}
