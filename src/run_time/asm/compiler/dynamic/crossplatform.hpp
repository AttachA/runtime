...
namespace art {
	...
	void Compiler::DynamicCompiler::remove(const ValueIndexPos& value_index) { CASM& a = compiler.a;
		BuildCall b(a, 1);
		b.lea_valindex({ compiler.static_map, compiler.values }, value_index);
		b.finalize(universalRemove);
	}

	void Compiler::DynamicCompiler::copy(const ValueIndexPos& to, const ValueIndexPos& from) { CASM& a = compiler.a;
		BuildCall b(a, 2);
	...
	void Compiler::DynamicCompiler::move(const ValueIndexPos& to, const ValueIndexPos& from) { CASM& a = compiler.a;
		BuildCall b(a, 2);
		b.lea_valindex({ compiler.static_map, compiler.values }, to);
		b.lea_valindex({ compiler.static_map, compiler.values }, from);
	...
	void Compiler::DynamicCompiler::sum(const ValueIndexPos& to, const ValueIndexPos& src) { CASM& a = compiler.a;
		BuildCall b(a, 2);
		b.lea_valindex({ compiler.static_map, compiler.values }, to);
	...
	void Compiler::DynamicCompiler::sub(const ValueIndexPos& to, const ValueIndexPos& src) { CASM& a = compiler.a;
		BuildCall b(a, 2);
		b.lea_valindex({ compiler.static_map, compiler.values }, to);
		b.lea_valindex({ compiler.static_map, compiler.values }, src);
		b.finalize((ValueItem&(ValueItem::*)(const ValueItem&))(&ValueItem::operator-=));
	...
	void Compiler::DynamicCompiler::mul(const ValueIndexPos& to, const ValueIndexPos& src) { CASM& a = compiler.a;
		BuildCall b(a, 2);
		b.lea_valindex({ compiler.static_map, compiler.values }, to);
		b.lea_valindex({ compiler.static_map, compiler.values }, src);
		b.finalize((ValueItem&(ValueItem::*)(const ValueItem&))(&ValueItem::operator*=));
	...
	void Compiler::DynamicCompiler::div(const ValueIndexPos& to, const ValueIndexPos& src) { CASM& a = compiler.a;
		BuildCall b(a, 2);
		b.lea_valindex({ compiler.static_map, compiler.values }, to);
		b.lea_valindex({ compiler.static_map, compiler.values }, src);
		b.finalize((ValueItem&(ValueItem::*)(const ValueItem&))(&ValueItem::operator/=));
	...
	void Compiler::DynamicCompiler::rest(const ValueIndexPos& to, const ValueIndexPos& src) { CASM& a = compiler.a;
		BuildCall b(a, 2);
		b.lea_valindex({ compiler.static_map, compiler.values }, to);
		b.lea_valindex({ compiler.static_map, compiler.values }, src);
		b.finalize((ValueItem&(ValueItem::*)(const ValueItem&))(&ValueItem::operator%=));
	...
	void Compiler::DynamicCompiler::bit_xor(const ValueIndexPos& to, const ValueIndexPos& src) { CASM& a = compiler.a;
		BuildCall b(a, 2);
		b.lea_valindex({ compiler.static_map, compiler.values }, to);
		b.lea_valindex({ compiler.static_map, compiler.values }, src);
		b.finalize((ValueItem&(ValueItem::*)(const ValueItem&))(&ValueItem::operator^=));
	...
	void Compiler::DynamicCompiler::bit_and(const ValueIndexPos& to, const ValueIndexPos& src) { CASM& a = compiler.a;
		BuildCall b(a, 2);
		b.lea_valindex({ compiler.static_map, compiler.values }, to);
		b.lea_valindex({ compiler.static_map, compiler.values }, src);
		b.finalize((ValueItem&(ValueItem::*)(const ValueItem&))(&ValueItem::operator&=));
	...
	void Compiler::DynamicCompiler::bit_or(const ValueIndexPos& to, const ValueIndexPos& src) { CASM& a = compiler.a;
		BuildCall b(a, 2);
		b.lea_valindex({ compiler.static_map, compiler.values }, to);
		b.lea_valindex({ compiler.static_map, compiler.values }, src);
		b.finalize((ValueItem&(ValueItem::*)(const ValueItem&))(&ValueItem::operator|=));
	...
	void Compiler::DynamicCompiler::bit_not(const ValueIndexPos& to) { CASM& a = compiler.a;
		BuildCall b(a, 1);
		b.lea_valindex({compiler.static_map, compiler.values}, to);
		b.finalize((ValueItem&(ValueItem::*)())(&ValueItem::operator!));
	}

	void Compiler::DynamicCompiler::bit_left_shift(const ValueIndexPos& to, const ValueIndexPos& src) { CASM& a = compiler.a;
		BuildCall b(a, 2);
		b.lea_valindex({ compiler.static_map, compiler.values }, to);
		b.lea_valindex({ compiler.static_map, compiler.values }, src);
		b.finalize((ValueItem&(ValueItem::*)(const ValueItem&))(&ValueItem::operator<<=));
	...
	void Compiler::DynamicCompiler::bit_right_shift(const ValueIndexPos& to, const ValueIndexPos& src) { CASM& a = compiler.a;
		BuildCall b(a, 2);
		b.lea_valindex({ compiler.static_map, compiler.values }, to);
		b.lea_valindex({ compiler.static_map, compiler.values }, src);
		b.finalize((ValueItem&(ValueItem::*)(const ValueItem&))(&ValueItem::operator>>=));
	...
	void Compiler::DynamicCompiler::call(const ValueIndexPos& fn_symbol, CallFlags flags) { CASM& a = compiler.a;
		BuildCall b(a, 0);
	...
	void Compiler::DynamicCompiler::call(const ValueIndexPos& fn_symbol, CallFlags flags, const ValueIndexPos& res) { CASM& a = compiler.a;
		BuildCall b(a, 0);
		if(fn_symbol.pos == ValuePos::in_constants) {
			if(flags.always_dynamic) {
				b.setArguments(5);
	...
	void Compiler::DynamicCompiler::call_local(const ValueIndexPos& fn_symbol, CallFlags flags) { CASM& a = compiler.a;
		if(fn_symbol.pos == ValuePos::in_constants) {
	...
	void Compiler::DynamicCompiler::call_local(const ValueIndexPos& fn_symbol, CallFlags flags, const ValueIndexPos& res) { CASM& a = compiler.a;
		if(fn_symbol.pos == ValuePos::in_constants) {
			uint64_t index = (uint64_t)compiler.values[fn_symbol.index + compiler.static_map.size()];
			auto& fn = compiler.build_func->localFn(index);
			if (flags.async_mode) {
	...
	void Compiler::DynamicCompiler::call_local_and_ret(const ValueIndexPos& fn_symbol, CallFlags flags) { CASM& a = compiler.a;
		if(fn_symbol.pos == ValuePos::in_constants) {
			uint64_t index = (uint64_t)compiler.values[fn_symbol.index + compiler.static_map.size()];
			auto& fn = compiler.build_func->localFn(index);
			if (flags.async_mode) {
