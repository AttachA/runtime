    ...
    namespace exception {
        ...
        ValueItem* get_current_exception_description() {
            ...
            if (current_ex_info.ex_ptr == nullptr) {
                ...
                case EXCEPTION_ACCESS_VIOLATION:
                    description =
                        "Program attempted to " + art::ustring(current_ex_info.ex_data_0 ? "write" : "read") + (current_ex_info.ex_data_1 < UINT16_MAX ? " in null pointer region. 0x" : " in non mapped region. 0x") + string_help::hexstr(current_ex_info.ex_data_1);
                    break;
                case EXCEPTION_STACK_OVERFLOW:
