...
namespace DynamicCall {
	...
	public:
		template<class F>
		FunctionCall(F to_call_func, const FunctionTemplate& func_template, bool allow_type_convert = false) : templ(func_template) {
			used_this = false; this_pointer = nullptr;
			func = (PROC)to_call_func;
			atc = allow_type_convert;
			holder.reserveArguments(max_arguments = templ.arguments.size());
			current_argument = 0;
		}
		FunctionCall(PROC to_call_func, void* this_ptr, const FunctionTemplate& func_template, bool allow_type_convert = false) : templ(func_template) {
			used_this = true;
