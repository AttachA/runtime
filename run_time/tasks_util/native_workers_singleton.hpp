...
class NativeWorkersSingleton {
    ...
    run_time::tasks::util::hill_climb hill_climb;
    run_time::threading::mutex hill_climb_mutex;
    ...
    NativeWorkersSingleton(){
        ...
        for(ptrdiff_t i = run_time::threading::thread::hardware_concurrency()/3 + 1; i > 0; i--)
    ...
    std::pair<uint32_t, uint32_t> proceed_hill_climb(double sample_seconds){
        const uint32_t max_threads = run_time::threading::thread::hardware_concurrency();
        std::lock_guard<run_time::threading::mutex> lock(hill_climb_mutex);
    ...
    void dispatch() {
        ...
        std::unique_lock<run_time::threading::mutex> lock(hill_climb_mutex);
